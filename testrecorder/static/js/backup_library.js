    ...
    try {
        ...
        if (response.ok) {
            const selected_playlistsData = await response.json();
             // console.log('selected_playlistsData 3647', selected_playlistsData)
            let playlistsDict = selected_playlistsData.playlist_videos;
            const playlistIds = Object.keys(playlistsDict);
            // console.log('playlistid detail 3649:', playlistIds)
            ...
            function LoadVideos(start, end) {
                ...
                for (let i = start; i < end; i++) {
                    ...
                    let video_listId = playlistIds[i];
                     // console.log('video_listId 3671:', video_listId)
                    let video_detail = playlistsDict[video_listId];
                    // console.log('Video Detail 3673:', video_detail)
                    const {
                        videoId,
                        privacyStatus,
                        videoTitle,
...
//                     console.log('Video Detail 3686:', video_detail)
                    ...
                    videoDiv.addEventListener('click', async () => {
                        ...
                        // Pass the entire video_detail object to the play function
...
async function play(videoObject) {
    ...
    setTimeout(() => {
        ...
        document.getElementById('video_viewCounts').textContent = videoObject.video_viewCount;
    }, 50);
}
...
function updateVideoInfo(videoObject) {
    ...
    privacy.textContent = `Playlist: ${videoObject.privacy}`;
...
async function play_first_video() {
    ...
    if (videos.children.length > 0) {
        ...
        for (let i = 0; i < videos.children.length; i++) {
            ...
            const video_viewCount = videoDiv.getAttribute('data-video-viewcount');
            const video_likeCount = videoDiv.getAttribute('data-video-likecount');
            const video_commentCount = videoDiv.getAttribute('data-video-commentcount');
